...
public:
  ...
  int run(geometry_msgs::PoseStamped &msg, int seq, ros::Time last_request);
};
...
void Awesomo::subscribeToAtim(void) {
  ...
  this->landing_subscriber =
    this->node.subscribe(ATIM_POSE_TOPIC, 10, &Awesomo::atimCallback, this);
}
...
void Awesomo::publishPositionControllerMessage(
  geometry_msgs::PoseStamped &msg, int seq, ros::Time time) {
  ...
  // rpy attitude -> used for simulator due to difficulty of getting a
  // consistent rpy out of a quaterniond
