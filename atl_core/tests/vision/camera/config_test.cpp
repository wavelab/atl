...
TEST(CameraConfig, load) {
  ...
  ASSERT_TRUE(atl::cvMatIsEqual(expected, config.camera_matrix));

  expected = cv::Mat(1, 5, CV_64F, distortion_coefficients);
  ASSERT_TRUE(atl::cvMatIsEqual(expected, config.distortion_coefficients));

  expected = cv::Mat(3, 3, CV_64F, rectification_matrix);
  ASSERT_TRUE(atl::cvMatIsEqual(expected, config.rectification_matrix));

  expected = cv::Mat(3, 4, CV_64F, projection_matrix);
  ASSERT_TRUE(atl::cvMatIsEqual(expected, config.projection_matrix));
  // clang-format on
