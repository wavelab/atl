...
For AprilTag pose estimation we have optimized and tested our solution using the Optitrack motion capture system to compare state estimates produced by our optimization, a demo of the AprilTag pose estimation can be seen in this 
\href{https://www.youtube.com/watch?v=BBMlQphrqCY&index=19&list=PLk5z6lLnKFd6BD9EXWa09hqf6SNQhn59R}{video}.

One major issue highlighted in Ling \cite{Ling2014} is that due to the relatively long processing time of the AprilTags library, the estimated state obtained at a time $t$ does not actually represent the current state due to a time lag. This time lag as outlined in \cite{Ling2014} and defined in \ref{Detection Delay} must be accounted for when using the camera to estimate state. We measured this lag using Optitrack motion capture system to provide a ground truth for the state from which we can estimate the estimated state.  
...
Before implementing our own PID position controller, we initially assumed that we could reuse PX4 firmware's own position controller and feed in the pose estimation of the quadrotor relative to the AprilTag observed using an onboard camera, we however met countless problems with regards to the code required to issue position commands, and instability issues related to the time lag induced when extracting a pose from an AprilTag. We therefore implemented our own PID position controller discussed in Section~\ref{subsec:position_controller}, and through many iterations of manually tuning the PID gains we found the following parameters yielded the best control behaviour.
...
Due to lack of time, we have not be able to complete the solution to autonomously land a quadrotor onto a moving platform. However, we have optimized the AprilTag pose estimation process and implemented our own position controller, what remains is the pose estimation of the quadrotor inbetween the pose estimation of the AprilTag. This is needed because even though we can obtain an accurate pose estimate of the quadrotor just by observing the AprilTag with a camera, the quadrotor can easily fly out of the AprilTag's view and lose track its own pose estimate, this is especially dangerous in quadrotors where a pose estimate is vital to its stability and navigation. It is our goal in the next coming weeks that we will implement a filter to estimate and combine the AprilTag and position controller to complete the autonomous landing solution.
